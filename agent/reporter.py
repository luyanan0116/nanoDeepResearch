...
class Reporter:
    def __init__(self, llm_client: Union[OpenAIClient, AnthropicClient], locale: str = "en-US"):
        template_dir = os.path.join(os.path.dirname(__file__), "..", "prompt")
        env = Environment(loader=FileSystemLoader(template_dir))
        
        # Load and render the template
        template = env.get_template("reporter.md")
        self.system_prompt = template.render(locale=locale)
