...
class Node(Enum):
    ...
    CODE = auto()
    REPORTER = auto()
    END = auto()  # Terminal state to exit the state machine
...
class StateMachine:
    ...
    def __init__(
        ...
        self.transitions = {
            ...
            Node.END: [],  # No transitions from END state
        }
    ...
    def _research_team_action(self) -> None:
        ...
        """
        # get the first step that has not been executed
        current_plan = self.state.get("current_plan")
        # if all steps have been executed, generate the report
